...
export async function GET() {
  ...
  return Response.json({ mode, horizons: [{ horizon: "90 days", actions: [gaps[0].nextAction, gaps[1].nextAction] }, { horizon: "6 months", actions: ["Ship a quantified portfolio case study", "Request target-role calibration"] }, { horizon: "12 months", actions: ["Apply to WT-IC3 product design roles"] }], gaps, compensation: { fourYearTotal: formatCompensation(compensation.fourYearTotal, "NTD"), realIndex: compensation.realIndex, evidence: "DEMO DATA" } });
}
