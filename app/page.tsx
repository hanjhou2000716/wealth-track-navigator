...
function ProfileWorkspace({ onBack }: { onBack: () => void }) {
  ...
  const ingestFile = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; setIngestionError(null); const form = new FormData(); form.append("file", file); const response = await fetch("/api/profile/ingest", { method: "POST", body: form }); const body = await response.json().catch(() => ({})); if (!response.ok) { setIngestionError(typeof body.error === "string" ? body.error : "檔案目前無法解析"); return; } setProfile(body.profile); setParsed(true); };
  const saveProfile = () => { window.localStorage.setItem("wealth-track-profile-v1", JSON.stringify({ profile, resumeText })); setSaved(true); };
