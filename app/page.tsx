  ...
  return (
    <main className="shell">
      ...
      <section className="content">
        <header className="topbar"><div><p className="eyebrow">2026 年 8 月 15 日・星期六 <span className="live-dot" /> 分析編號 #024</p><h1>讓你的下一步，<em>持續複利。</em></h1></div><div className="top-actions"><button className="icon-button" aria-label="通知">♢</button><button className="profile-button">JH <span>⌄</span></button></div></header>
        ...
        <section className="trust-panel panel" id="evidence"><div><p className="eyebrow">為什麼是這個結果？／證據信任</p><h2>先看來源，再相信分數。</h2><p>信任分數由來源可靠度、資料新鮮度、證據覆蓋與跨來源一致性計算；不是模型自行填寫的信心百分比。</p><div className="trust-source">來源：{analysis?.trust?.source ?? "使用者示範履歷"}・{analysis?.trust?.sourceTier ?? "第一級主要來源"}・{analysis?.trust?.freshness ?? "新鮮"}</div></div><div className="trust-factors">{[["來源可靠度", analysis?.trust?.breakdown.sourceReliability ?? 100], ["資料新鮮度", analysis?.trust?.breakdown.freshness ?? 100], ["證據覆蓋", analysis?.trust?.breakdown.evidenceCoverage ?? 88], ["跨來源一致性", analysis?.trust?.breakdown.crossSourceAgreement ?? 85]].map(([label, value]) => <div className="trust-factor" key={label as string}><span>{label}</span><strong>{value}%</strong><i><b style={{ width: `${value}%` }} /></i></div>)}</div></section>
...
function ModuleView({ active, onBack }: { active: string; onBack: () => void }) {
  ...
  const [compensation, setCompensation] = useState<{ years: { year: number; equity: number; bonus: number; signOn: number }[]; fourYearTotalFormatted: string; realIndex: number | null; regions?: { region: string; fourYearTotalFormatted: string; colAdjustedTotalFormatted: string | null; realIndex: number | null }[]; evidence: string } | null>(null);
  ...
  const content: Record<string, { kicker: string; title: string; intro: string; cards: { label: string; value: string; detail: string; tone: string }[] }> = {
    ...
    Path: {
      ...
      cards: [
        { label: "目標樣本", value: "500", detail: "要求分析的履歷數", tone: "violet" },
        { label: "可用履歷歷史", value: "—", detail: "示範模式未連接資料供應商", tone: "amber" },
    ...
    Network: {
      kicker: "人脈 / 證據閘門內推",
      title: "一個有效引薦，始於真實訊號。",
      intro: "不虛構校友、不抓取個人檔案。連接合法資料供應商前，示範模式會誠實顯示空狀態。",
      cards: [
        { label: "已驗證個人檔案", value: "0", detail: "沒有可靠資料供應商證據", tone: "cyan" },
        { label: "關係狀態", value: "未驗證", detail: "聯絡前先確認關係", tone: "amber" },
        { label: "下一步", value: "加入來源", detail: "連接已授權資料供應商", tone: "violet" },
      ],
    },
    Comp: {
      kicker: "薪酬 / 跨區套利",
      title: "比較你真正能留下的總包。",
      intro: "拆開股權歸屬、簽約金與購買力，因為名目薪資從來不是完整故事。",
      cards: [
        { label: "目前方案", value: "NT$1.08M", detail: "示範輸入・台灣", tone: "cyan" },
        { label: "四年名目總額", value: "NT$4.52M", detail: "底薪＋獎金＋股權歸屬", tone: "violet" },
        { label: "實質指數", value: "100", detail: "需要資料供應商證據", tone: "lime" },
      ],
    },
    Plan: {
      kicker: "計畫 / 職涯策略",
  ...
  const cards = active === "Market value" && marketValue ? [
    ...
    { label: "可用轉職樣本", value: String(trajectory.sampleSize), detail: "示範種子資料；不足以做市場推論", tone: "violet" },
    { label: "最佳跳板分數", value: String(trajectory.transitions[0]?.springboardScore ?? "—"), detail: trajectory.transitions[0]?.to ?? "尚無可用跳板", tone: "cyan" },
    { label: "資料品質", value: trajectory.transitions[0]?.quality === "DEMO_INSUFFICIENT" ? "示範資料不足" : "已驗證", detail: "需要合法職涯歷史 Provider 才能升級", tone: "amber" },
    ...
    { label: "挑戰路徑", value: `${radar.find((item) => item.track === "STRETCH")?.match ?? "—"}`, detail: radar.find((item) => item.track === "STRETCH")?.gaps.join("、") || "需要更多職責證據", tone: "violet" },
    ...
    { label: "第四年股權", value: compensation.years[3] ? String(Math.round(compensation.years[3].equity)) : "—", detail: "依歸屬時程計算", tone: "amber" },
  ] : view.cards;
  return <main className="module-shell"><header className="module-top"><button className="back-button" onClick={onBack}>← 返回總覽</button><div className="module-mode"><span className="demo-dot" /> 示範資料・證據模式</div></header><section className="module-hero"><p className="eyebrow">{view.kicker}</p><h1>{view.title}</h1><p>{view.intro}</p></section><section className="module-cards">{cards.map((card) => <article className="module-card panel" key={card.label}><div className={`metric-icon ${card.tone}`}>◎</div><small>{card.label}</small><strong>{card.value}</strong><p>{card.detail}</p></article>)}</section>{active === "Comp" && compensation?.regions && <section className="region-grid"><div className="region-heading"><p className="eyebrow">跨區薪酬比較</p><h2>同一份總包，落地後不一樣。</h2></div><div className="region-cards">{compensation.regions.map((region) => <article className="region-card panel" key={region.region}><strong>{region.region}</strong><span>{region.fourYearTotalFormatted}</span><small>購買力調整：{region.colAdjustedTotalFormatted ?? "資料目前無法取得"}・實質指數 {region.realIndex ?? "—"}</small></article>)}</div></section>}<section className="module-detail panel"><div><p className="eyebrow">為什麼是這個結果？</p><h2>先看證據，再做推論。</h2><p>這個畫面上的數字不是使用者看不懂的黑箱：它們來自使用者資料、可重現的規則，或明確標示為目前不可取得。建議與事實保持分離。</p></div><div className="evidence-list"><span>✓ 使用者提供的履歷</span><span>✓ 標準化職級架構</span><span>○ {providerSummary.replace("Provider", "資料供應商")}</span></div></section></main>;
}
...
function ProfileWorkspace({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    try {
      const raw = window.localStorage.getItem("wealth-track-profile-v1");
      if (!raw) return;
      const stored = JSON.parse(raw) as { profile?: typeof demoProfile; resumeText?: string };
      if (stored.profile?.name && stored.profile.location && stored.profile.summary) setProfile(stored.profile);
      if (typeof stored.resumeText === "string") setResumeText(stored.resumeText);
      if (typeof stored.resumeText === "string") setOriginalResume(stored.resumeText);
      setSaved(true);
    } catch {
      setSaved(false);
    }
  ...
  const parseResume = async () => { if (!resumeText.trim()) return; const response = await fetch("/api/profile/parse", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ text: resumeText }) }); if (!response.ok) return; const body = await response.json(); setProfile(body.profile); setOriginalResume(body.originalText || resumeText); setParsed(true); };
  const ingestFile = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; setIngestionError(null); const form = new FormData(); form.append("file", file); const response = await fetch("/api/profile/ingest", { method: "POST", body: form }); const body = await response.json().catch(() => ({})); if (!response.ok) { setIngestionError(typeof body.error === "string" ? body.error : "檔案目前無法解析"); return; } setProfile(body.profile); setOriginalResume(body.originalText || ""); setResumeText(body.originalText || ""); setParsed(true); };
  const saveProfile = () => { window.localStorage.setItem("wealth-track-profile-v1", JSON.stringify({ profile, resumeText: originalResume || resumeText })); setSaved(true); };
  return <main className="module-shell"><header className="module-top"><button className="back-button" onClick={onBack}>← 返回總覽</button><div className="module-mode"><span className="demo-dot" /> 示範資料・可編輯履歷</div></header><section className="profile-editor"><div className="profile-editor-heading"><div><p className="eyebrow">履歷 / 結構化資料</p><h1>讓市場看懂你的經驗。</h1><p>先修正結構化履歷，再計算任何分數或建議。</p></div><button className="save-button" onClick={saveProfile}>{saved ? "已儲存於本機 ✓" : "儲存修正"}</button></div><section className="paste-box panel"><div><p className="eyebrow">快速導入</p><h2>貼上履歷文字</h2><p>支援 PDF、DOCX、TXT 與中英文混合文字；解析後仍可人工修正，系統不會自行補造經歷。</p><label className="file-input">匯入履歷檔案<input type="file" accept=".pdf,.docx,.txt,text/plain,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={ingestFile} /></label>{ingestionError && <p className="ingestion-error" role="alert">{ingestionError}</p>}</div><textarea value={resumeText} onChange={(event) => setResumeText(event.target.value)} placeholder="請貼上履歷文字，例如：姓名、職稱、專案與技能…" /><div className="paste-actions"><button className="parse-button" onClick={parseResume}>解析並建立結構化履歷</button>{parsed && <span>✓ 已解析，請檢查下方欄位</span>}</div></section><section className="resume-compare"><article className="panel resume-original"><p className="eyebrow">原始履歷</p><pre>{originalResume || "完成導入後，這裡會保留原始文字供比對。"}</pre></article><article className="panel resume-structured"><p className="eyebrow">結構化履歷</p><p><strong>{profile.name}</strong>・{profile.location}</p><p>{profile.summary}</p><div className="chip-list">{profile.skills.map((skill) => <span key={skill}>{skill}</span>)}</div></article></section><div className="editor-grid"><label>姓名<input value={profile.name} onChange={(event) => update("name", event.target.value)} /></label><label>所在地<input value={profile.location} onChange={(event) => update("location", event.target.value)} /></label><label className="wide">個人摘要<textarea value={profile.summary} onChange={(event) => update("summary", event.target.value)} /></label></div><div className="profile-columns"><div><p className="eyebrow">工作經歷</p><article className="employment-card panel"><strong>{profile.employment[0].role}</strong><span>{profile.employment[0].company}・{profile.employment[0].startedAt}</span><p>{profile.employment[0].scope}</p><small>影響力證據：{profile.employment[0].impact}</small></article></div><div><p className="eyebrow">技能與證據</p><div className="chip-list">{profile.skills.map((skill) => <span key={skill}>{skill}</span>)}</div><div className="editor-note">ⓘ 使用者提供的欄位屬於第一級主要來源證據；我們不會自行捏造缺少的數字。修正內容只保存在此瀏覽器，尚未寫入遠端資料庫。</div></div></div></section></main>;
}
